# dcomix step 2/2: put the 4K comic atlas into robot.glb, flatten the
# materials (metallic 0 everywhere — it's a drawing, and robot.js shades
# everything with the soft plastic matcap), re-export with JPEG textures,
# render a preview.
#   blender --background --python helper/scripts/comic_ai_apply.py
import bpy
import math
import os
import re
...
ATLAS = os.path.join(ROOT, "helper/blender/robot_atlas_comic_ai.png")
...
SHIP = 2048  # author at 4K, ship display-matched: the panel renders ~500-800
# CSS px, so a Lanczos-downscaled 2K atlas is visually identical on-site at
# half the payload and a quarter of the VRAM

atlas = bpy.data.images.load(ATLAS)
atlas.name = "robot_atlas_comic_ai"
atlas.scale(SHIP, SHIP)
atlas.pack()
...
# 4K comic art (flat fills + strokes) packs small as JPEG; PNG would balloon
bpy.ops.export_scene.gltf(
    filepath=GLB, export_format="GLB",
    export_image_format="JPEG", export_jpeg_quality=85,
)
print("EXPORTED", GLB, os.path.getsize(GLB), "bytes")
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(ROOT, "helper/blender/robot_comic_ai.blend"))
...
scene.render.filepath = os.path.join(ROOT, "helper/renders/robot_comic_ai.png")
bpy.ops.render.render(write_still=True)
print("DONE comic-ai apply")
